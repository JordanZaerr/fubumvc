...
{
    public class EnvelopeRejected : MessageLogRecord
    {
        public EnvelopeRejected()
        {
        }

        public EnvelopeRejected(EnvelopeToken envelope, ChannelNode node, string message)
        {
            Envelope = envelope;
            Message = message;
        ...
        public override MessageRecord ToRecord()
        {
            ...
            {
                Message = $"Envelope Rejected - {Message}"
            };
        }
        ...
        {
            return $"Rejected {Envelope} to {Uri} ({Key})";
        }
    }
}
